-- =====================================================
-- JKUAT Innovation Club - Notification System (Clean Install)
...
BEGIN
    ...
    ELSE
        RAISE NOTICE 'Users table not found - foreign key not added';
    END IF;
END $$;
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'users') THEN
        ALTER TABLE notification_templates ADD CONSTRAINT fk_notification_templates_created_by FOREIGN KEY (created_by) REFERENCES users(id);
        ALTER TABLE notification_templates ADD CONSTRAINT fk_notification_templates_modified_by FOREIGN KEY (last_modified_by) REFERENCES users(id);
    END IF;
END $$;
...
SELECT 'notification_templates table created' AS status;

-- =====================================================
-- STEP 5: Create notification_campaigns table
-- =====================================================
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'users') THEN
        ALTER TABLE notification_campaigns ADD CONSTRAINT fk_notification_campaigns_created_by FOREIGN KEY (created_by) REFERENCES users(id);
    END IF;
END $$;
...
SELECT 'notification_campaigns table created' AS status;

-- =====================================================
-- STEP 6: Create notification_deliveries table
-- =====================================================
...
SELECT 'notification_deliveries table created' AS status;
...
CREATE FUNCTION get_notification_stats(p_start_date DATE, p_end_date DATE)
RETURNS TABLE (
    total_sent BIGINT,
...
SELECT 'Database functions created' AS status;
...
SELECT 
    ...
    (SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = 'public' AND (table_name LIKE 'notification%' OR table_name = 'push_subscriptions')) AS tables_created,
    (SELECT COUNT(*) FROM information_schema.routines WHERE routine_schema = 'public' AND routine_name LIKE '%notification%') AS functions_created;
