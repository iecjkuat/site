-- =============================================
-- JKUAT Innovation Club - Partnerships & Opportunities Schema (Simple Version)
...
-- Step 3: Add new columns to existing opportunities table (one by one to avoid issues)
ALTER TABLE opportunities ADD COLUMN IF NOT EXISTS category_id UUID;
...
ALTER TABLE opportunities ADD COLUMN IF NOT EXISTS updated_by UUID;

-- Step 4: Add foreign key constraints (after columns are created)
DO $$
BEGIN
    ...
    IF NOT EXISTS (
        ...
        ALTER TABLE opportunities ADD CONSTRAINT opportunities_category_id_fkey 
            FOREIGN KEY (category_id) REFERENCES opportunity_categories(id);
    END IF;

    -- Add foreign key for updated_by if it doesn't exist
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'opportunities_updated_by_fkey' 
        AND table_name = 'opportunities'
    ) THEN
        ALTER TABLE opportunities ADD CONSTRAINT opportunities_updated_by_fkey 
...
-- Step 5: Create opportunity_applications table
...
-- Add unique constraint for applications (one application per user per opportunity)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'opportunity_applications_user_opportunity_unique' 
        AND table_name = 'opportunity_applications'
    ) THEN
        ALTER TABLE opportunity_applications ADD CONSTRAINT opportunity_applications_user_opportunity_unique 
...
-- Step 6: Create opportunity_bookmarks table
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'opportunity_bookmarks_user_opportunity_unique' 
        AND table_name = 'opportunity_bookmarks'
    ) THEN
        ALTER TABLE opportunity_bookmarks ADD CONSTRAINT opportunity_bookmarks_user_opportunity_unique 
            UNIQUE(opportunity_id, user_id);
    END IF;
EXCEPTION
    WHEN OTHERS THEN
...
-- Step 7: Create opportunity_views table
...
-- Step 8: Create user_opportunity_preferences table
CREATE TABLE IF NOT EXISTS user_opportunity_preferences (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id UUID REFERENCES users(id) ON DELETE CASCADE,
    preferred_types VARCHAR(50)[],
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'user_opportunity_preferences_user_unique' 
        AND table_name = 'user_opportunity_preferences'
    ) THEN
        ALTER TABLE user_opportunity_preferences ADD CONSTRAINT user_opportunity_preferences_user_unique 
            UNIQUE(user_id);
    END IF;
EXCEPTION
    WHEN OTHERS THEN
...
-- Step 9: Create opportunity_notifications table
...
-- Step 11: Insert initial opportunity categories
...
-- Step 12: Insert sample partnership organizations
...
-- Success message
SELECT 'Partnerships & Opportunities schema applied successfully!' as result;